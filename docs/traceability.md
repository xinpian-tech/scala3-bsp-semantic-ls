...
| Cross-file navigation callback | `symbol_definition` resolver over the immutable snapshot, forward-closure pruning, `pc-plugins.json` compiler-plugin steering over the live vtable (the generic `pcNavTestPlugin` fixture) | `crates/ls-jvm/tests/live_definition.rs` (flake check `pc-definition`), `crates/ls-jvm/tests/live_pcplugin.rs` (flake check `pc-plugin-load`), `crates/ls-server/tests/live_pc.rs` (flake check `pc-server-definition`) |
...
| Island parity (retained Scala) | `pc-plugins.json` loading, plugin SPI self-test/disable-on-throw, `PcTargetConfig`, `Utf16Text`, LRU instance cap, SemanticDB-flag stripping | mill suites under `modules/ls-pc/test` (`PluginManagerSuite`, `PcQuerySuite`, `PcSymbolSearchSuite`, `Utf16TextSuite`, `PcWorkerManagerSuite`, `CompilerPluginConfigSuite`), `modules/ls-pc-host/test`; the loaded-plugin boundary leg is `crates/ls-jvm/tests/live_pcplugin.rs` (flake check `pc-plugin-load`) |
...
| FULL-workspace zaozi macro-navigation e2e | the UNTRIMMED zaozi checkout (every CIRCT/Panama module in the model) built inside zaozi's OWN dev shell (`nix develop $ZAOZI_SRC`: CIRCT/MLIR install paths, its JDK), driven by the same headless-Neovim client against the PACKAGED server (`nix build .#default`, the wrapper's baked defaults) with NO `pc-plugins.json` — zaozi's own in-build `zaozi-compiler-plugin` (batch SemanticDB enhancement + interactive navigation phases) reaches the island through the build's `-Xplugin` scalacOptions over `buildTarget/scalacOptions`: the full-model health gates (ready / session compile under an env-tunable budget with timeout-retry across the BSP per-request bound / reindex / doctor target count), then the dynamic-bundle-field probes — HARD gates: `textDocument/definition` and `hover` at a real `io.<field>` access land on the `val <field> = Aligned/Flipped(...)` declaration (same-file anchor `UIntSpecIO.a` in `zaozi/tests/src/UIntSpec.scala`, cross-file anchor `QueueIO.empty` from `stdlib/src/dwbb/Queue.scala` to `stdlib/src/Queue.scala`; expected lines grep-computed at run time), and `references` at each field DEFINITION covers the dynamic-access sites (≥ 2 sites incl. the access file — the plugin-injected SemanticDB occurrences served by the index); INFO-only: `workspace/symbol` on the field name (the `E2E INFO: dynamic-field …` count line) | `it/nvim/e2e.lua` (the `LS_NVIM_FULL=1` branch), `scripts/it-nvim-zaozi-full.sh`, `docs/deployment.md` §5.7 |
...
8. **The Scala real-repo zaozi probe script retired.** The deleted script
   drove the removed Scala assembly's ad-hoc probe flags; the zaozi navigation
   obligations moved to the editor-level e2e rows above. The pinned, patched
   zaozi source stays exposed as `ZAOZI_SRC` (flake input) for the e2e and for
   manual real-repo validation with any LSP client.
...
12. **The in-repo zaozi PC plugin retired; `pc-plugins.json` proven by a
   generic fixture.** zaozi's own `zaozi-compiler-plugin` now ships the
   interactive PC navigation phase in-build, reaching the island through the
   build's `-Xplugin` scalacOptions (proven by the FULL-workspace e2e row,
   which runs WITHOUT any `pc-plugins.json`), so the shipped
   `ls-zaozi-pcplugin` module, its jar in the package `share/`, and the
   `live_zaozi` boundary test were removed. The `pc-plugins.json`
   `compilerPlugins` mechanism remains a product feature and is now proven by
   a fixture this repo owns: the check-only `pcNavTestPlugin` module
   (`modules/ls-pc-navtestplugin`, never shipped) steers a marker Dynamic
   shape it defines itself, loaded into the live island via a workspace
   `pc-plugins.json` and observed over the real vtable in
   `crates/ls-jvm/tests/live_pcplugin.rs` (flake check `pc-plugin-load`,
   replacing `pc-zaozi`); the zaozi-specific single-buffer steering suites
   moved with the plugin to zaozi's repo.
