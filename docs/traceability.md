...
| FULL-workspace zaozi macro-navigation e2e | the UNTRIMMED zaozi checkout (every CIRCT/Panama module in the model) built inside zaozi's OWN dev shell (`nix develop $ZAOZI_SRC`: CIRCT/MLIR install paths, its JDK), driven by the same headless-Neovim client against the PACKAGED server (`nix build .#default`, the wrapper's baked defaults) with the shipped zaozi PC plugin wired through the workspace `pc-plugins.json`: the full-model health gates (ready / session compile under an env-tunable budget with timeout-retry across the BSP per-request bound / reindex / doctor target count), then the dynamic-bundle-field probes — HARD gates: `textDocument/definition` and `hover` at a real `io.<field>` access land on the `val <field> = Aligned/Flipped(...)` declaration (same-file anchor `UIntSpecIO.a` in `zaozi/tests/src/UIntSpec.scala`, cross-file anchor `QueueIO.empty` from `stdlib/src/dwbb/Queue.scala` to `stdlib/src/Queue.scala`; expected lines grep-computed at run time); INFO-only until the index-side zaozi SemanticDB compiler plugin lands: `references` at the field definition and `workspace/symbol` on the field name (the `E2E INFO: dynamic-field …` count lines, to flip to hard gates then) | `it/nvim/e2e.lua` (the `LS_NVIM_FULL=1` branch), `scripts/it-nvim-zaozi-full.sh`, `docs/deployment.md` §5.7 |
