...
| Semantic tokens (full + range) | `textDocument/semanticTokens/full` + `/range` gated like hover (`require_semanticdb` hard error, `withPcBuffer` → null — spec `SemanticTokens \| null`, an open buffer always answers a stream, empty included); the island's `[start, end)` UTF-16 offset nodes positioned against the OPEN BUFFER TEXT (`line-index` line split, astral-safe UTF-16 columns), unclassified `-1`/zero-width/hypothetical multi-line nodes dropped (dotty nodes are single-line symbol-name spans — pinned), sorted and LSP delta-encoded with no `resultId` (`full.delta` is NOT advertised — no delta handler exists); `/range` slices the node list server-side before encoding so the first token's deltas restart from the document origin | encoder units in `crates/ls-server/src/pc_lsp.rs`, gate + dispatch in `crates/ls-server/src/services.rs`, wire snapshots in `crates/ls-server/tests/pc_wire.rs`, cold fallbacks in `it/lsp-blackbox/test_semantic_tokens.py`, real-island stream facts in `it/nvim/e2e.lua` |
