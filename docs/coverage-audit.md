...
| modules/ls-pc/test/src/ls/pc/corpus/CompletionCaseCorpusSuite.scala | CORPUS | island-only: enum match/case exhaustiveness cases from scala3 3.8.4 `tests/completion/{CompletionCaseSuite,CompletionMatchSuite}.scala` (same-buffer Scala shapes: children carry source positions, so the exhaustive-match sorter never consults the toplevels seam) |
| modules/ls-pc/test/src/ls/pc/corpus/CompletionMatchCorpusSuite.scala | CORPUS | island-only: `match`/`match (exhaustive)` cases from scala3 3.8.4 `tests/completion/CompletionMatchSuite.scala` exercising the `definitionSourceToplevels` ordering seam — the Java-enum shapes (`java.nio.file.AccessMode`, classfile children with no source positions) consult the mocked seam in both the `match@@` and lambda `case@@` shapes with scrambled-order pins (the facade twin of the `crates/ls-jvm/tests/live_definition.rs` live leg), the no-mock fallback keeps compiler declaration order, and the scala-library Option cases pin that POSITIONED children skip the seam entirely |
...
| modules/ls-pc/test/src/ls/pc/corpus/SemanticTokensCorpusSuite.scala | CORPUS | island-only: semantic-tokens cases from scala3 3.8.4 `tests/tokens/SemanticTokensSuite.scala` — a curated 24-case subset favoring Scala 3 syntax (enums, end markers, export, extension blocks, structural types, @main, for-comprehensions), rendered through the harness port of `TestSemanticTokens.pcSemanticString` (same `getTypePriority` tie-break) |
| modules/ls-pc/test/src/ls/pc/corpus/DiagnosticsCorpusSuite.scala | CORPUS | island-only: PC diagnostics cases from scala3 3.8.4 `tests/DiagnosticProviderSuite.scala` (error/warning/mixed/attached quick-fix data) plus `tests/ExplainDiagnosticProviderSuite.scala` under a second `-explain` target, through the facade `diagnostics` path (the `pc_diagnostics` op) |
| modules/ls-pc/test/src/ls/pc/corpus/AutoImportsCorpusSuite.scala | CORPUS | island-only: auto-import cases from scala3 3.8.4 `tests/edit/AutoImportsSuite.scala` — 17 of 33 curated (drops: Ammonite/worksheet/scala-cli filename semantics; workspace-search-dependent cases — the corpus facade has no workspace symbol index) — through the facade `autoImports` carrier (candidate package lists + applied edits, incl. the `ju`/`mutable` symbol-prefix renames) |
| modules/ls-pc/test/src/ls/pc/corpus/InsertInferredTypeCorpusSuite.scala | CORPUS | island-only: insert-inferred-type cases from scala3 3.8.4 `tests/edit/InsertInferredTypeSuite.scala` — 16 of 69 curated favoring Scala 3 syntax (toplevel defs, named tuples, enums, backticks) plus the val/var/def/tuple matrix and the auto-import edits — through the facade `codeAction(InsertInferredType)` carrier |
| modules/ls-pc/test/src/ls/pc/corpus/AutoImplementAbstractMembersCorpusSuite.scala | CORPUS | island-only: implement-abstract-members cases from scala3 3.8.4 `tests/edit/AutoImplementAbstractMembersSuite.scala` — 12 of 46 curated (braceless templates, extension methods, givens, end markers, object creation, overloads) — through the facade `codeAction(ImplementAbstractMembers)` carrier |
| modules/ls-pc/test/src/ls/pc/corpus/ExtractMethodCorpusSuite.scala | CORPUS | island-only: extract-method cases from scala3 3.8.4 `tests/edit/ExtractMethodSuite.scala` — 10 of 23 curated; the facade carrier has no separate extraction-anchor position, so 8 cases characterize the derived-anchor semantics (`PcFacade.extractionAnchor`: in-place extraction at the selection's first-line statement) with observed valid output, 2 match upstream verbatim |
| modules/ls-pc/test/src/ls/pc/corpus/InlineValueCorpusSuite.scala | CORPUS | island-only: inline-value cases from scala3 3.8.4 `tests/edit/InlineValueSuite.scala` — 8 of 37 curated incl. BOTH refusal families (non-local, shadowed-variable) exercising the `DisplayableException` refusal-as-data mapping — through the facade `codeAction(InlineValue)` carrier |
| modules/ls-pc/test/src/ls/pc/corpus/ConvertToNamedArgumentsCorpusSuite.scala | CORPUS | island-only: the FULL scala3 3.8.4 `tests/edit/ConvertToNamedArgumentsSuite.scala` (6 cases) incl. backticked names, multi-clause `new` applies, and the java-object refusal — through the facade `codeAction(ConvertToNamedArguments)` carrier with explicit argument indices |
| modules/ls-pc/test/src/ls/pc/corpus/ConvertToNamedLambdaParametersCorpusSuite.scala | CORPUS | island-only: convert-to-named-lambda-parameters cases from scala3 3.8.4 `tests/edit/ConvertToNamedLambdaParametersSuite.scala` — 5 of 12 curated (single/multi underscore, nested lambdas, match-with-wildcard) — through the facade `codeAction(ConvertToNamedLambdaParameters)` carrier |
| modules/ls-pc/test/src/ls/pc/corpus/InsertInferredMethodCorpusSuite.scala | CORPUS | island-only: insert-inferred-method cases from scala3 3.8.4 `tests/edit/InsertInferredMethodSuite.scala` — 8 of 29 curated (argument inference, no-args, custom types, lambdas, backticked names, extension methods) — through the facade `codeAction(InsertInferredMethod)` carrier |
| modules/ls-pc/test/src/ls/pc/SemanticTokensLegendSuite.scala | ISLAND | the cross-language legend parity pin: the vendored `scala.meta.internal.pc.SemanticTokens.TokenTypes`/`TokenModifiers` lists printed + asserted against the SAME pinned values as the Rust `legend::TOKEN_TYPES`/`TOKEN_MODIFIERS` (`crates/ls-server/src/pc_lsp.rs`), with the shared golden anchors ("method" = 13, "declaration" = 0) |
