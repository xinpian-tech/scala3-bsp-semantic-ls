...
class RawInstruction(tokens: Seq[Token]):
  ...
  override def toString: String =
    if importInstruction.nonEmpty then
      s"import instruction: ${importInstruction.get._2} from ${importInstruction.get._1}"
    else if importInstructionSet.nonEmpty then s"import set: ${importInstructionSet.get}"
    else if pseudoInstruction.nonEmpty then
      s"$name(pseudo ${pseudoInstruction.get._2} from ${pseudoInstruction.get._1})"
